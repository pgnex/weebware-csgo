...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_first_use, float bg_alpha_override, ImGuiWindowFlags flags)
{
	...
	if (size_first_use.x != 0.0f || size_first_use.y != 0.0f)
		ImGui::SetNextWindowSize(size_first_use, ImGuiCond_Always);
...
// Render a rectangle shaped with optional rounding and borders
void render_frame_button(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding, int flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;


	if (flags & ImGuiButtonFlags_Outlined) {
		window->DrawList->AddRectFilled(p_min - ImVec2(1, 1), p_max + ImVec2(1, 1), ImGui::ColorConvertFloat4ToU32(imgui_custom::ConvertFromRGBA(ImVec4(0.f, 0.f, 0.f, 255.f))), rounding);
		window->DrawList->AddRectFilled(p_min, p_max, ImGui::ColorConvertFloat4ToU32(imgui_custom::ConvertFromRGBA(ImVec4(60.f, 60.f, 60.f, 255.f))), rounding);
		ImVec4(0.161f, 0.502f, 0.725f, 1.0f);
	}
	window->DrawList->AddRectFilled(p_min + ImVec2(1, 1), p_max - ImVec2(1, 1), fill_col, rounding);

	const float border_size = g.Style.FrameBorderSize;

	if (border && border_size > 0.0f)
	{
		window->DrawList->AddRect(p_min + ImVec2(1, 1), p_max + ImVec2(1, 1), ImGui::GetColorU32(ImGuiCol_BorderShadow), rounding, ImDrawCornerFlags_All, border_size);
		window->DrawList->AddRect(p_min, p_max, ImGui::GetColorU32(ImGuiCol_Border), rounding, ImDrawCornerFlags_All, border_size);
	}
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	RenderNavHighlight(bb, id);
	render_frame_button(bb.Min, bb.Max, col, true, style.FrameRounding, flags);
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, int flags)
{
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	ImVec2 temptMin = frame_bb.Min, temptMax = frame_bb.Max;
	temptMin.y += 6.f; temptMax.y -= 6.f;
	temptMin.x += 4.f; temptMax.x -= 4.f;

	window->DrawList->AddRectFilled(temptMin - ImVec2(1, 1), temptMax + ImVec2(1, 1), ColorConvertFloat4ToU32(imgui_custom::ConvertFromRGBA(ImVec4(0.f, 0.f, 0.f, 255.f))), style.FrameRounding);

	window->DrawList->AddRectFilled(temptMin, temptMax, frame_col, style.FrameRounding);
	...
	ImVec2 tempt_new_max = temptMax;
	tempt_new_max.x = (grab_bb.Max.x + grab_bb.Min.x) * 0.5f;
	window->DrawList->AddRectFilled(temptMin, tempt_new_max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
	// grab_bb
	return value_changed;
}
...
void custom_render_frame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border, float rounding, ImVec2 pad)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	window->DrawList->AddRectFilled(p_min + pad, p_max - pad, fill_col, rounding);
}

bool ImGui::Checkbox(const char* label, bool* v, bool use_tick)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const float pad = ImMax(1.0f, (float)(int)(check_sz / 6.0f)) + 3.f;

	// Render Frame.
	window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad, pad) - ImVec2(1, 1), check_bb.Max - ImVec2(pad, pad) + ImVec2(1, 1), ColorConvertFloat4ToU32(imgui_custom::ConvertFromRGBA(ImVec4(0.f, 0.f, 0.f, 255.f))), style.FrameRounding);

	custom_render_frame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding, ImVec2(pad, pad));
...
bool ImGui::CheckboxFlags(const char* label, unsigned int* flags, unsigned int flags_value)
{
	...
	bool pressed = Checkbox(label, &v, true);
	if (pressed)
	{
		if (v)
			*flags |= flags_value;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
	...
	if (!(flags & ImGuiComboFlags_NoPreview)) {
		window->DrawList->AddRectFilled(frame_bb.Min - ImVec2(1, 1), frame_bb.Max + ImVec2(1, 1), ColorConvertFloat4ToU32(imgui_custom::ConvertFromRGBA(ImVec4(0.f, 0.f, 0.f, 255.f))), style.FrameRounding, ImDrawCornerFlags_Left);
		window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Max.y), frame_col, style.FrameRounding, ImDrawCornerFlags_Left);
	}
	if (!(flags & ImGuiComboFlags_NoArrowButton))
	{
		window->DrawList->AddRectFilled(ImVec2(frame_bb.Max.x - arrow_size, frame_bb.Min.y), frame_bb.Max, frame_col, style.FrameRounding, (w <= arrow_size) ? ImDrawCornerFlags_All : ImDrawCornerFlags_Right);
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
	if (ImGui::Begin("ImGui Metrics", p_open))
	{
		...
		ImGui::Checkbox("Show clipping rectangles when hovering draw commands", &show_clip_rects, true);
		ImGui::Separator();
