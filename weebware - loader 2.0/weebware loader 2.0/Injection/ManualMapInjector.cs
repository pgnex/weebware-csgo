...
namespace Injection {
    internal class ManualMapInjector {
        ...
        private IntPtr GetRemoteModuleHandleA(string module) {
            ...
            var dwSize = (uint)Marshal.SizeOf(typeof(https_www_youtube_com_watch_v_nNpW8KZOVhw));
            ...
            if (dwStatus >= 0) {
                if (pbi.Value.MATT != IntPtr.Zero) {
                    UIntPtr dwBytesRead;
                    uint pebLdrAddress;

                    if (Imports.ReadProcessMemory(_hProcess, pbi.Value.MATT + 12 /*peb.Ldr*/, out pebLdrAddress, out dwBytesRead)) {
        ...
        private bool ProcessSection(char[] name, IntPtr baseAddress, IntPtr remoteAddress, ulong rawData, ulong virtualAddress, ulong rawSize, ulong virtualSize, uint protectFlag) {
            ...
            if (
                !Imports.NtWriteVirtualMemory(_hProcess, new IntPtr(remoteAddress.ToInt64() + (long)virtualAddress), new IntPtr(baseAddress.ToInt64() + (long)rawData),
        ...
        [Obfuscation(Feature = "virtualization", Exclude = false)]
        public IntPtr Inject(byte[] data, string strKey = "") {
            var handle = new GCHandle();

            byte[] buffer;
            bool needsDecryption = strKey.Length > 0;
            if (needsDecryption) {
